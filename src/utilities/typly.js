// From https://github.com/webcc/typly/blob/master/lib/typly.js // 
...
module.exports = {
    ...
    isImplementationOf: function (_object, _class) {
        if (_object instanceof _class) {
            return true;
        } else {
            if (this.isNull(_object) || !this.isObject(_object)) {
                return false;
            } else {
                let objectPropertyNames = Object.getOwnPropertyNames(_object);
                let classPropertyNames = Object.getOwnPropertyNames(
                    new _class()
                );
                return (
                    classPropertyNames.length ===
                    _.intersection(classPropertyNames, objectPropertyNames)
                        .length
                );
            }
        }
    },
    assertImplementationOf: function (value, prototype) {
        if (!this.isImplementationOf(value, prototype)) {
    ...
    assertArray: function (value, fn) {
        if (arguments.length === 1) {
            ...
            if (!this.isArray(value, fn)) {
                throw new TypeError(
                    '[typly] Invalid comparison function or type used in array'
                );
            }
        }
        return true;
    ...
    assertDateString: function (value) {
        if (!this.isDateString(value)) {
            throw createTypeError(
                value,
                'Invalid string representation of Date object'
            );
        }
        return true;
    },
